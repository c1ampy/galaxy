...
static void menu_render_frame(const Button* buttons, const size_t button_count, const int width, const int height, const int high_scores[3], const int difficulty) {
	...
	outtextxy(width / 2 - textwidth(diff_buf) / 2, 560, diff_buf);
	...
	outtextxy(width / 2 - textwidth(line_easy) / 2, 580, line_easy);
	outtextxy(width / 2 - textwidth(line_normal) / 2, 600, line_normal);
	outtextxy(width / 2 - textwidth(line_hard) / 2, 620, line_hard);
...
int render_draw_difficulty_menu(const int width, const int height, const int difficulty, const int fps) {
	...
	while (true) {
		...
		outtextxy(width / 2 - textwidth(current_buf) / 2, 250, current_buf);
...
int render_draw_wasted_page(const GameplayVisualState* state, const int high_scores[3], const int fps) {
	...
	while (true) {
		...
		outtextxy(state->width / 2 - textwidth(L"WASTED") / 2, 180, L"WASTED");
		...
		outtextxy(state->width / 2 - textwidth(score_buf) / 2, 300, score_buf);
		...
		outtextxy(state->width / 2 - textwidth(high_buf) / 2, 300 + 30, high_buf);
