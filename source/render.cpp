...
int render_load_gameplay_textures(
	...
	int ok5 = load_internal_texture(&g_render_textures.enemy_bullet, &g_render_textures.enemy_bullet_ok, enemy_bullet_path);
...
void render_draw_current_frame(const GameplayVisualState* state) {
	...
	if (g_render_textures.enemy_bullet_ok && state->enemy_bullet_list) {
		for (Node* enemy_bullet_node = state->enemy_bullet_list->head->next; enemy_bullet_node; enemy_bullet_node = enemy_bullet_node->next) {
			const Object* enemy_bullet = (const Object*)enemy_bullet_node->data;
			putimage(enemy_bullet->x, enemy_bullet->y, &g_render_textures.enemy_bullet);
		}
	}
